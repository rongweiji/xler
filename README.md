...
- Enable recording via CLI with:
  ...
  - Adjust capture cadence via `camera.fps` in `xler.yaml`; you can override FPS from the CLI with `--fps <number>`. Override device/output paths via `--camera-left`, `--camera-right`, or `--camera-output-dir`.
